...
class ASTGeneration(MT22Visitor):
    ...
    def visitIntList(self, ctx:MT22Parser.IntListContext):
        lst = [x.getText() for x in ctx.INT_LIT()]
        return lst
    ...
    def visitVar_decl(self, ctx: MT22Parser.Var_declContext):
        ...
        return [VarDecl(varlist[x], vartype, initExp[x]) for x in range(len(varlist))]
    ...
    def visitLiteral(self, ctx:MT22Parser.LiteralContext):
        if ctx.INT_LIT():
            return IntegerLit(ctx.INT_LIT())
        elif ctx.FLOAT_LIT():
            return FloatLit(ctx.FLOAT_LIT())
        elif ctx.STR_LIT():
            return StringLit(ctx.STR_LIT())
        else:
            return BooleanLit(ctx.FALSE()) if ctx.FALSE() else BooleanLit(ctx.TRUE())
    ...
    def visitFor_stmt(self, ctx:MT22Parser.For_stmtContext):
        init = self.visitExpression(ctx.expression()[0])
        cond = self.visitExpression(ctx.expression()[1])
        update = self.visitExpression(ctx.expression()[2])
